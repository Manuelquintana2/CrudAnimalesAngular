...
export class PaisesService {
  ...
  traerPaises(): Observable<Pais[]> {
    return this.http.get<Pais[]>(this.apiUrl).pipe(
      map(paises => 
        paises.filter(pais => pais.region === 'Americas' || pais.region === 'Asia') // Filtrar por región
      ),
      map(paises => 
        paises.sort((a, b) => a.name.common.localeCompare(b.name.common)) // Ordenar alfabéticamente
      )
    );
  }
